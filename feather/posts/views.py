...
def post_detail(request, post_id):
    ...
    # answer_to = Comment.objects.filter(pk=comments.answer_to).first()
...
@login_required
def add_answer(request, post_id, comment_id):
    post = Post.objects.filter(pk=post_id).first()
    # answer_to = Comment.objects.filter(pk=comment_id).first()
    form = CommentForm(request.POST or None)
    if form.is_valid():
        comment = form.save(commit=False)
        comment.author = request.user
        comment.post = post
        comment.answer_to = Comment.objects.filter(pk=comment_id).first()
        comment.save()
    else:
        raise ValueError()
    return redirect('posts:post_detail', post_id=post_id)
